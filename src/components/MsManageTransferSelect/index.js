...
const mapStateToProps = (state) => {
  const result = state[namespace].allUserList;
  return {
    result
  };
};
...
export default class MsManageTransferSelect extends Component {
  state = {
    tableDataLeft: [],
    leftDataTmp: [],
    tableDataRight: [],
  ...
  componentDidMount() {
    ...
    const columnsLeft = [
      //{ title: '门店编号', dataIndex: 'userMapId', key: 'userMapId' },
      { title: '未选', dataIndex: 'userName', key: 'userName' }
    ];
    const columnsRight = [
      //{ title: '门店编号', dataIndex: 'userMapId', key: 'userMapId' },
      { title: '已选', dataIndex: 'userName', key: 'userName' }
    ];
    this.setState({
  ...
  initList() {
    ...
    for (let i = 0; i < tableData.length; i++) {
      ...
      for (let j = 0; j < selectedShopList.length; j++) {
        if (row.userMapId == selectedShopList[j].managerId) {
  ...
  changeList = (type = 'left') => {
    ...
    if (typeof this.props.onChange === 'function') {
      ...
      for (var index = 0; index < tableDataRight.length; index++) {
        userMapIdList.push(tableDataRight[index].userMapId)
      }

      this.props.onChange(userMapIdList)
    }
  }
